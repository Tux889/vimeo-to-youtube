...
const { databaseUtil, printUtil, vimeoUtil, countUtil } = require('./util')
...
const prompt = () => inquirer.prompt({
  ...
  choices: [
    ...
    constants.CHOICE_PRINT_LISTCOUNT,
    constants.CHOICE_REFRESH,
    constants.CHOICE_RESET_DOWNLOADING,
    constants.CHOICE_RESET_FAILED,
    new inquirer.Separator(),
  ...
  .then(answer => {
    ...
    switch (answer.action) {
      ...
      case constants.CHOICE_RESET_DOWNLOADING: return databaseUtil.resetDownloadingVideos()
        ...
        .then(() => prompt())
      case constants.CHOICE_RESET_FAILED: return databaseUtil.resetDownloadFailedVideos()
        .then(n => logger.info(`${n} video(s) reset`))
        .then(() => prompt())
      case constants.CHOICE_PRINT_LIST: return printUtil.all().then(() => prompt())
      case constants.CHOICE_PRINT_LISTCOUNT: return countUtil.count().then(() => prompt())
