...
const downloadWorker = ({ video, dest }) => {
  ...
  const foldername = video.parent_folder !== null ? `${video.parent_folder.name}` : ''
  const destWithFolder = path.join(dest, foldername)
  const absolutePath = path.join(dest, foldername, filename)
  ...
  return {
    ...
    task: () => new Observable(observer =>
      databaseUtil.markVideoAsDowloading(video)
        ...
        .then(() => dl(downloadOption.link, destWithFolder, { filename })
